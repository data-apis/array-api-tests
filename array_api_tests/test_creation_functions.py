...
from hypothesis.strategies import integers, floats, one_of, none, booleans, just, data
...
@given(xps.scalar_dtypes(), xps.array_shapes(), data())
def test_full_like(dtype, shape, data):
    a = data.draw(xps.arrays(dtype, shape))
    fill_value = data.draw(xps.from_dtype(dtype))
    kwargs = data.draw(sampled_from([{}, {'dtype': dtype}]))
    ...
    if is_float_dtype(a_like.dtype) and isnan(asarray(fill_value)):
        ...
        assert all(equal(a_like, asarray(fill_value, dtype=dtype))), "full_like() array did not equal the fill value"
